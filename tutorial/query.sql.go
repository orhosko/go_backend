...
SELECT id, team, points, wins, draws, losses, goal_diff FROM standing
WHERE team = ?
LIMIT 1
`

func (q *Queries) GetStanding(ctx context.Context, team interface{}) (Standing, error) {
	row := q.db.QueryRowContext(ctx, getStanding, team)
	var i Standing
	err := row.Scan(
		&i.ID,
		&i.Team,
